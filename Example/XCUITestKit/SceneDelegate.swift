...
@available(iOS 13.0, *)
extension SceneDelegate: LiveResetHostDelegate {
    func didReceiveReset() {
        reset(withScene: currentScene)
    }

    func reset(withScene windowScene: UIWindowScene?) {
        tearDown()

        //        Make and create AppDependencies if needed

        window = injectNewRootViewController(to: createWindow(withScene: windowScene))
    }
